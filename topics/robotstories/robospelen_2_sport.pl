state(robospelen_2_sport, s1, say).
text(robospelen_2_sport, s1, "Hé, ik heb opeens een heel goed idee!").
next(robospelen_2_sport, s1, "true", s2).

state(robospelen_2_sport, s2, question).
stateConfig(robospelen_2_sport, s2, [type=yesno, context='answer_yesno']).
...
state(robospelen_2_sport, s3nee6, question).
stateConfig(robospelen_2_sport, s3nee6, [type=input, context="integer", options=['vijf', 'vier', 'drie', 'twee', 'een'], umVariable=getal_0_tot_10]).
...
state(robospelen_2_sport, s3nee_met_getal1, say).
...
state(robospelen_2_sport, s3nee_zonder_getal1, say).
