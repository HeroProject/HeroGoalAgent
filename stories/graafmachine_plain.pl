...
next(story, s9, "true", s11).

state(story, s11, say).
text(story, s11, "Mijn nichtje was mijn favorite graafmachine.").
...
state(story, s16, say).
text(story, s16, "Mijn nichtje werkt op waterstof maar ik weet niet precies wat dat is.").
...
next(story, s32, "true", s35).

state(story, s35, say).
text(story, s35, "Ik groef verder de modder weg.").
...
state(story, s38, say).
text(story, s38, "En af en toe gooide ik expres een beetje modder over mijn nichtje heen.").
...
next(story, s41, "true", s45).

state(story, s45, say).
text(story, s45, "Ik kon mijn arm niet meer bewegen, als ik hem probeerde op te halen, duwde ik in plaats van mijn arm mijn lichaam omhoog.").
...
state(story, s50, say).
text(story, s50, "En hij werd eerst heel boos dat ik mijn arm vast had laten zitten.").
next(story, s50, "true", s51).

state(story, s51, say).
text(story, s51, "En toen moest ik hem beloven dat ik vanaf nu voorzichtiger te werk zou gaan en toen ging hij me helpen met mijn arm losmaken.").
next(story, s51, "true", s52).

state(story, s52, say).
text(story, s52, "Hij blies met zijn stoom alle aarde om mijn arm weg.").
next(story, s52, "true", s57g).
...
next(story, s74, "true", s75c).

state(story, s75c, say).
text(story, s75c, "Mijn nichtje rolde naar me toe.").
...
next(story, s76, "true", s79).

state(story, s79, say).
text(story, s79, "En toen wilde ik eerst super boos op haar worden want ik had hem gevonden en zij was hem aan het stelen. Maar toen zei opa dat ik dat niet mocht doen.").
...
state(story, s85, say).
text(story, s85, "En mijn nichtje werkt daar nu.").
