...
stateConfig(test, s5f, [type=mc, response=touch, options = ["blauw", "geel", "rood", "paars"], context='answer_color', key='favoriteColor', defaultAnswer="rood"]). % "wit", "oranje", "rood", "roze", "blauw", "geel", "groen", "paars", "zwart", "bruin"
...
state(test, s6f, say).
text(test, s6f, "Dan kies ik voor %favoriteColor%."). % favoriteColor is a variable that is replaced with an answer given by user for key 'favoriteColor' (see s5).
next(test, s6f, 'true', s7).
