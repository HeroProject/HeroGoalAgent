...
moveConfig(robotkleren, s10, [type=input, context="clothing", options=['badjas', 'robotpak', 't-shirt', 'rok', 'trui'], umVariable=kledingstuk]).
...
moveConfig(robotkleren, s11niets8, [type=input, context="color", options=['rood', 'geel', 'blauw', 'groen'], umVariable=kleur_kledingstuk]).
text(robotkleren, s11niets8, "Welke kleur zou je dan kiezen?").
next(robotkleren, s11niets8, "fail", s11fail_kleur1).
next(robotkleren, s11niets8, "_others", s11kleur_gekozen1).
...
text(robotkleren, s11badjas2, "Die kan ik op de RoboSpelen tussen de wedstrijden door aan!").
next(robotkleren, s11badjas2, "true", s12).

move(robotkleren, s11niets9, question).
moveConfig(robotkleren, s11niets9, [type=input, context="color", options=['rood', 'geel', 'blauw', 'groen'], umVariable=kleur_kledingstuk]).
text(robotkleren, s11niets9, "Welke kleur badjas zou jij voor mij kiezen?").
next(robotkleren, s11niets9, "fail", s11fail_kleur1).
next(robotkleren, s11niets9, "_others", s11kleur_gekozen_badjas1).
...
moveConfig(robotkleren, s11een_kledingstuk4, [type=input, context="color", options=['rood', 'geel', 'blauw', 'groen'], umVariable=kleur_kledingstuk]).
...
next(robotkleren, s11een_kledingstuk4, "_others", s11kleur_gekozen1).
...
next(robotkleren, s11flitsend1, "true", s12).

move(robotkleren, s12, question).
moveConfig(robotkleren, s12, [type=yesno, context='answer_yesno', umVariable=gelijke_kleuren]).
text(robotkleren, s12, "he, is %kleur_kledingstuk% niet jouw lievelingskleur?").
next(robotkleren, s12, "answer_yes", s13gelijk1).
next(robotkleren, s12, "answer_no", s13ongelijk1).
next(robotkleren, s12, "answer_dontknow", s13fail_gelijk1).
next(robotkleren, s12, "fail", s13fail_gelijk1).

move(robotkleren, s13fail_gelijk1, say).
text(robotkleren, s13fail_gelijk1, "Ik vind ook meer dan één kleur mooi.").
next(robotkleren, s13fail_gelijk1, "true", s13fail_gelijk2).

move(robotkleren, s13fail_gelijk2, say).
text(robotkleren, s13fail_gelijk2, "Maar %kleur_kledingstuk% vind ik supermooi, dus dankjewel!").
next(robotkleren, s13fail_gelijk2, "true", s14).

move(robotkleren, s13gelijk1, say).
text(robotkleren, s13gelijk1, "Hoera, dan heb ik dat goed onthouden!").
next(robotkleren, s13gelijk1, "true", s13gelijk2).

move(robotkleren, s13gelijk2, say).
text(robotkleren, s13gelijk2, "Wat lief dat ik jouw lievelingskleur aan mag!").
next(robotkleren, s13gelijk2, "true", s14).

move(robotkleren, s13ongelijk1, say).
text(robotkleren, s13ongelijk1, "O, dan heb ik dat verkeerd onthouden!").
next(robotkleren, s13ongelijk1, "true", s13ongelijk2).

move(robotkleren, s13ongelijk2, say).
text(robotkleren, s13ongelijk2, "Maar %kleur_kledingstuk% vind ik supermooi, dus dankjewel!").
next(robotkleren, s13ongelijk2, "true", s14).

move(robotkleren, s14, say).
text(robotkleren, s14, "Misschien kun je later wel robotkleren gaan ontwerpen!").
