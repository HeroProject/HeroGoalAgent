...
move(ho1_greeting, s1, say).
text(ho1_greeting,  s1, "Hallo! Ik ben Hero en ik ben een robot.").
next(ho1_greeting,  s1, "true",  s2).

move(ho1_greeting, s2, say).
moveConfig(ho1_greeting, s2, [waitTimer=3000]).
...
move(ho1_tutorial_speech, s4, say).
text(ho1_tutorial_speech,  s4, "Laten we een keertje oefenen. Hier komt de eerste vraag.").
next(ho1_tutorial_speech,  s4, "true",  s5).

move(ho1_tutorial_speech, s5, question).
moveConfig(ho1_tutorial_speech, s5, [type=quiz, context='answer_color', correctAnswer=["blauw", "donkerblauw"], inputModality=[speech=3], additionalAttempt=true, fast=yes]).
...
move(ho1_tutorial_speech, s5f, say).
...
text(ho1_tutorial_feet, s9y, "Ik hou ook van chocola! Goed gedrukt trouwens.").
...
moveConfig(ho1_tutorial_feet, s9f, [next='FrontTactilTouched']).
...
move(ho1_tutorial_question, s2, question).
moveConfig(ho1_tutorial_question, s2, [type=yesno, context='answer_yesno']).
...
move(ho1_animals, s3y, say).
text(ho1_animals,  s3y, "Leuk zeg.").
next(ho1_animals, s3y, "true", s4y).

move(ho1_animals, s4y, question).
moveConfig(ho1_animals, s4y, [type=input, context='answer_favorite_animal', options= ["Hond", "Kat", "Konijn", "Cavia", "Hamster", "Vis", "Vogel"], umVariable=huisdier]).
text(ho1_animals, s4y, "Welke dan?").
next(ho1_animals, s4y, 'success', s5).
next(ho1_animals, s4y, 'fail', s5f).

move(ho1_animals, s3n, question).
moveConfig(ho1_animals, s3n, [type=input, context='answer_favorite_animal', options= ["Hond", "Kat", "Konijn", "Cavia", "Hamster", "Vis", "Vogel"], umVariable=huisdier]).
...
move(ho1_colors, s2n, say).
text(ho1_colors,  s2n, "Als je een kleur zou moeten kiezen.").
next(ho1_colors, s2n, "true", s3).

move(ho1_colors, s3, question).
moveConfig(ho1_colors, s3, [type=input, context='answer_color', options= ["Groen", "Paars", "Rood", "Roze", "Blauw", "Oranje", "Geel"], umVariable=fav_color]).
text(ho1_colors, s3, "Wat is dan jouw lievelings kleur?").
next(ho1_colors, s3, 'success', s4).
next(ho1_colors, s3, 'fail', s7).

move(ho1_colors, s4, say).
...
move(ho1_family, s6n, say).
text(ho1_family,  s6n, "Je mag er wel eetje van mij lenen hoor.").
next(ho1_family,  s6n, "true",  s7).

move(ho1_family, s7, question).
moveConfig(ho1_family, s7, [type=yesno, context='answer_yesno']).
...
move(ho1_goodbye, s4y, say).
text(ho1_goodbye,  s4y, "Leuk om te horen, ik klets graag.").
next(ho1_goodbye,  s4y, "true",  s5y).

move(ho1_goodbye, s4d, say).
text(ho1_goodbye,  s4d, "Oke").
next(ho1_goodbye,  s4d, "true",  s4n).

move(ho1_goodbye, s4n, question).
moveConfig(ho1_goodbye, s4n, [type=openend, context='answer_open', inputModality=[speech=1]]).
