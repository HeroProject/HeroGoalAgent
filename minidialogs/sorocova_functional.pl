...
moveConfig(sor_begroeting_1, s4, [waitTimer=3000]).
...
text(sor_tutorial,  s22, "Deze wordt soms ook rood").
leds(sor_tutorial, s22, direct, ["RightFootLeds", "LeftFootLeds"], ["wit", "rood"]).
next(sor_tutorial,  s22, "true",  s23).

move(sor_tutorial, s23, say).
text(sor_tutorial,  s23, "Het kan zijn dat ik je antwoord verkeerd heb verstaan.").
next(sor_tutorial,  s23, "true",  s24).

move(sor_tutorial, s24, say).
text(sor_tutorial,  s24, "Zolang dat lampje rood is, kun je op die teen drukken om mij te verbeteren. \pau=400\ ").
next(sor_tutorial,  s24, "true",  s25).
...
moveConfig(sor_tutorial, s29, [eventListener=['misheard'='LeftBumperPressed'], goTimer=5000]).
display(sor_tutorial, s29, "%math_given_answer%", always).
leds(sor_tutorial, s29, direct, ["LeftFootLeds"], ["rood"]).
...
set_max_duration(sor_naar_rekenen_1,  s1, 480000, 30000). % 8 minutes
...
set_max_duration(sor_naar_rekenen_2,  s1, 600000, 30000). % 10 minutes
...
set_max_duration(sor_naar_rekenen_3,  s1, 720000, 30000). % 12 minutes
